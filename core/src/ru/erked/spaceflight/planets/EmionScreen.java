...
public class EmionScreen implements Screen{
	...
	public EmionScreen(final StartSFlight game){
		this.game = game;
	}
	...
	public void show() {
		...
		backgroundSprite = RES.atlas.createSprite("emion");
		backgroundSprite.setBounds(0.0F, 0.0F, width, 2.0F*height);
		
		if(INF.planetLevel >= 20){
			timer = 60*60;
			...
			rocketTimer = CurPR.getCurRocket().getTimeBonus()*60 + 60;
			gemsToWin = 210 + (INF.planetLevel - CurPR.getCurPlanet().getLevel())*15;
		}
	...
	private void gemInit(){
		...
		for(int i=0;i<5;i++){
			for(int j=0;j<10;j++){
				gemMatrix[i][j] = rand.nextInt(5) + 1;
				...
				if(INF.planetLevel == 14){
					gemMatrix[4][0] = 9;
					gemMatrix[4][9] = 9;
				}else if(INF.planetLevel == 19){
					gemMatrix[4][0] = 9;
					gemMatrix[4][1] = 9;
					gemMatrix[4][8] = 9;
					gemMatrix[4][9] = 9;
		...
		if(!INF.lngRussian){
			...
			if(INF.planetLevel >= 20){
				...
				if(!isGameOver)
					...
					header.draw(batch, "Победa!", 0.5F*width - 0.5F*header.getWidth("Победа!"), 1.5F*height + 0.5F*header.getHeight("A"));
			}
			if(rocketS.getY() <= 1.0F*height && startTimer <= 0.0F){
				if(!isGameOver){
					text.draw(batch, score + " / " + gemsToWin, 
	...
	private boolean addition(){
		...
		for(int j=0;j<10;j++){
			if(gemMatrix[0][j] == 0){
				check = true;
				gemMatrix[0][j] = rand.nextInt(5) + 1;
