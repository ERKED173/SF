...
public class IngmarScreen implements Screen{
	...
	private ru.erked.spaceflight.tech.SFButtonA back;
	
	private ru.erked.spaceflight.tech.Rocket rocket;
	private ru.erked.spaceflight.tech.SFButtonS rocketS;
	private ru.erked.spaceflight.tech.SFButtonS rocketBall;
	private ru.erked.spaceflight.tech.SFButtonS rocketCircle;
	private ru.erked.spaceflight.tech.SFButtonS rocketBasic;
	private ru.erked.spaceflight.tech.SFButtonS rocketKinetic;
	private ru.erked.spaceflight.tech.SFButtonS rocketDelta;
	private ru.erked.spaceflight.tech.SFButtonS rocketInfinity;
	private Sprite[] fire;
	...
	private Sprite[][] amber;
	private Sprite[][] obsidian;
	private Sprite[][] quartz;
	private Sprite[][] granite;
	...
	private ArrayList<ru.erked.spaceflight.tech.ParticleFX[]> explosions;
	
	public IngmarScreen(final ru.erked.spaceflight.StartSFlight game){
		this.game = game;
	}
	...
	public void show() {
		...
		backgroundSprite = ru.erked.spaceflight.random.RES.atlas.createSprite("ingmar");
		backgroundSprite.setBounds(0.0F, 0.0F, width, 2.0F*height);
		
		if(ru.erked.spaceflight.random.INF.planetLevel >= 50){
			timer = 60*60;
			rocketTimer = ru.erked.spaceflight.tech.CurPR.getCurRocket().getTimeBonus()*60 + 60;
			gemsToWin = 0;
			isEndlessMode = true;
		}else{
			timer = 60*60 + (ru.erked.spaceflight.random.INF.planetLevel - ru.erked.spaceflight.tech.CurPR.getCurPlanet().getLevel())*5*60;
			rocketTimer = ru.erked.spaceflight.tech.CurPR.getCurRocket().getTimeBonus()*60 + 60;
			gemsToWin = 250 + (ru.erked.spaceflight.random.INF.planetLevel - ru.erked.spaceflight.tech.CurPR.getCurPlanet().getLevel())*15;
		}
		
		gems = ru.erked.spaceflight.random.RES.atlas.createSprite("gems");
		gems.setBounds(0.375F*width, 0.8F*height, 0.15F*height, 0.15F*height);
		clock = new Sprite[4];
		for(int i=0;i<4;i++){
			clock[i] = ru.erked.spaceflight.random.RES.atlas.createSprite("clock", i+1);
		...
		if(!ru.erked.spaceflight.random.INF.lngRussian){
			back = new ru.erked.spaceflight.tech.SFButtonA("continueI", "continueA", 0.3F*width, width - 0.315F*width, 0.875F*height, 5.97826F, 1.0F, -1);
		}else{
			back = new ru.erked.spaceflight.tech.SFButtonA("continueRI", "continueRA", 0.3F*width, width - 0.315F*width, 0.875F*height, 5.97826F, 1.0F, -1);
		}

		text = new ru.erked.spaceflight.tech.SFFont(30, Color.WHITE, 1, 2, Color.BLACK);
		header = new ru.erked.spaceflight.tech.SFFont(12, Color.WHITE, 4.0F, Color.BLACK);
	...
	private void gemInit(){
		...
		diamond = new Sprite[5][10];
		amber = new Sprite[5][10];
		obsidian = new Sprite[5][10];
		quartz = new Sprite[5][10];
		granite = new Sprite[5][10];
		gemMatrix = new int[5][10];
		for(int i=0;i<5;i++){
			for(int j=0;j<10;j++){
				gemMatrix[i][j] = rand.nextInt(8) + 1;
				
				if(ru.erked.spaceflight.random.INF.isBombActive)
					if(rand.nextInt(30) == 0)
						gemMatrix[i][j] += 10;
				if(ru.erked.spaceflight.random.INF.isColoumnActive)
					if(rand.nextInt(35) == 0)
						gemMatrix[i][j] += 20;
				if(ru.erked.spaceflight.random.INF.isLineActive)
					if(rand.nextInt(45) == 0)
						gemMatrix[i][j] += 30;
				if(ru.erked.spaceflight.random.INF.isTimeActive)
					if(rand.nextInt(50) == 0)
						gemMatrix[i][j] += 40;
				
				if(ru.erked.spaceflight.random.INF.planetLevel == 44){
					gemMatrix[3][0] = 9;
					gemMatrix[3][1] = 9;
					gemMatrix[4][0] = 9;
					gemMatrix[4][1] = 9;
					...
					gemMatrix[4][8] = 9;
					gemMatrix[4][9] = 9;
				}else if(ru.erked.spaceflight.random.INF.planetLevel == 49){
					gemMatrix[2][0] = 9;
					gemMatrix[3][0] = 9;
					gemMatrix[3][1] = 9;
					gemMatrix[4][0] = 9;
				...
				case 1:{
					topaz[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("topaz");
					break;
				}case 2:{
					sapphire[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("sapphire");
					break;
				}case 3:{
					ruby[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("ruby");
					break;
				}case 4:{
					emerald[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("emerald");
					break;
				}case 5:{
					diamond[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("diamond");
					break;
				}case 6:{
					amber[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("amber");
					break;
				}case 7:{
					obsidian[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("obsidian");
					break;
				}case 8:{
					quartz[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("quartz");
					break;
				}case 11:{
					topaz[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("topazB");
					break;
				}case 21:{
					topaz[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("topazC");
					break;
				}case 31:{
					topaz[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("topazL");
					break;
				}case 41:{
					topaz[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("topazT");
					break;
				}case 12:{
					sapphire[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("sapphireB");
					break;
				}case 22:{
					sapphire[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("sapphireC");
					break;
				}case 32:{
					sapphire[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("sapphireL");
					break;
				}case 42:{
					sapphire[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("sapphireT");
					break;
				}case 13:{
					ruby[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("rubyB");
					break;
				}case 23:{
					ruby[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("rubyC");
					break;
				}case 33:{
					ruby[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("rubyL");
					break;
				}case 43:{
					ruby[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("rubyT");
					break;
				}case 14:{
					emerald[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("emeraldB");
					break;
				}case 24:{
					emerald[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("emeraldC");
					break;
				}case 34:{
					emerald[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("emeraldL");
					break;
				}case 44:{
					emerald[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("emeraldT");
					break;
				}case 15:{
					diamond[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("diamondB");
					break;
				}case 25:{
					diamond[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("diamondC");
					break;
				}case 35:{
					diamond[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("diamondL");
					break;
				}case 45:{
					diamond[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("diamondT");
					break;
				}case 16:{
					amber[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("amberB");
					break;
				}case 26:{
					amber[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("amberC");
					break;
				}case 36:{
					amber[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("amberL");
					break;
				}case 46:{
					amber[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("amberT");
					break;
				}case 17:{
					obsidian[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("obsidianB");
					break;
				}case 27:{
					obsidian[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("obsidianC");
					break;
				}case 37:{
					obsidian[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("obsidianL");
					break;
				}case 47:{
					obsidian[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("obsidianT");
					break;
				}case 18:{
					quartz[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("quartzB");
					break;
				}case 28:{
					quartz[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("quartzC");
					break;
				}case 38:{
					quartz[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("quartzL");
					break;
				}case 48:{
					quartz[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("quartzT");
					break;
				}case 9:{
					granite[i][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("granite");
					break;
				}default:{
					break;
				}
		...
		explosions = new ArrayList<ru.erked.spaceflight.tech.ParticleFX[]>();
	}
	private void rocketsInit(){
		fire = new Sprite[2];
		for(int i=0;i<2;i++)
			fire[i] = ru.erked.spaceflight.random.RES.atlas.createSprite("fire",i+1);
		/***/
		rocketBall = new ru.erked.spaceflight.tech.SFButtonS("rocketBallI", "rocketBallA", 0.225F*height, 0.5F*width - 0.1125F*height, 2.5F*height, 0.52727F, 3.0F, -1);
		rocketCircle = new ru.erked.spaceflight.tech.SFButtonS("rocketCircleI", "rocketCircleA", 0.225F*height, 0.5F*width - 0.1125F*height, 2.5F*height, 0.52727F, 3.0F, -1);
		rocketBasic = new ru.erked.spaceflight.tech.SFButtonS("rocketBasicI", "rocketBasicA", 0.225F*height, 0.5F*width - 0.1125F*height, 2.5F*height, 0.32967F, 3.0F, -1);
		rocketKinetic = new ru.erked.spaceflight.tech.SFButtonS("rocketKineticI", "rocketKineticA", 0.175F*height, 0.5F*width - 0.0875F*height, 2.5F*height, 0.25F, 3.0F, -1);
		rocketDelta = new ru.erked.spaceflight.tech.SFButtonS("rocketDeltaI", "rocketDeltaA", 0.115F*height, 0.5F*width - 0.0575F*height, 2.5F*height, 0.1796875F, 3.0F, -1);
		rocketInfinity = new ru.erked.spaceflight.tech.SFButtonS("rocketInfinityI", "rocketInfinityA", 0.125F*height, 0.5F*width - 0.0625F*height, 2.5F*height, 0.1796875F, 3.0F, -1);
		/***/
		rocket = ru.erked.spaceflight.tech.CurPR.getCurRocket();
		if(rocket != null){
			if(rocket.equals(ru.erked.spaceflight.random.INF.rocketBall)){
				rocketS = rocketBall;
				for(int i=0;i<2;i++)
			...
			}else if(rocket.equals(ru.erked.spaceflight.random.INF.rocketCircle)){
				rocketS = rocketCircle;
				for(int i=0;i<2;i++)
					fire[i].setBounds(rocketS.getX() + 0.27625F*rocketS.getWidth(), rocketS.getY() + 0.2F*rocketS.getHeight(), 0.45F*rocketS.getWidth(), 0.9F*rocketS.getWidth());
			}else if(rocket.equals(ru.erked.spaceflight.random.INF.rocketBasic)){
				rocketS = rocketBasic;
				for(int i=0;i<2;i++)
			...
			}else if(rocket.equals(ru.erked.spaceflight.random.INF.rocketKinetic)){
			...
			}else if(rocket.equals(ru.erked.spaceflight.random.INF.rocketDelta)){
				rocketS = rocketDelta;
				for(int i=0;i<2;i++)
			...
			}else if(rocket.equals(ru.erked.spaceflight.random.INF.rocketInfinity)){
	...
	private ru.erked.spaceflight.tech.ParticleFX[] coloumn$LineExplosion(int i, int j, float timer){
		ru.erked.spaceflight.tech.ParticleFX[] explosion = new ru.erked.spaceflight.tech.ParticleFX[6];
		for(int g=0;g<6;g++){
			explosion[g] = new ru.erked.spaceflight.tech.ParticleFX(
	...
	private ru.erked.spaceflight.tech.ParticleFX[] bombExplosion(int i, int j, float timer){
		ru.erked.spaceflight.tech.ParticleFX[] explosion = new ru.erked.spaceflight.tech.ParticleFX[6];
		for(int g=0;g<6;g++){
			explosion[g] = new ru.erked.spaceflight.tech.ParticleFX(
	...
	private void interfaceDraw(float delta){
		if(!isGameOver){
			if(ru.erked.spaceflight.random.INF.elapsedTime % 30 == 0) clockIter++;
		...
		if(rocketS.getY() <= 1.0F*height && startTimer <= 0.0F && !isGameOver){
			if(score >= gemsToWin && gemsToWin != 0 && !isEndlessMode){
				if(!isTimeLeft){
					isTimeLeft = true;
					timeLeft = (int)(timer - ru.erked.spaceflight.random.INF.elapsedTime + time2)/60;
				}
				isGameOver = true;
				isVictory = true;
			}
			if(!isStartedTimer1){
				...
				time1 = ru.erked.spaceflight.random.INF.elapsedTime;
			}
			gems.draw(batch);
			clock[clockIter].draw(batch);
			if((rocketTimer - ru.erked.spaceflight.random.INF.elapsedTime + time1)/60 <= 0){
				if(!isStartedTimer2){
					isStartedTimer2 = true;
					time2 = ru.erked.spaceflight.random.INF.elapsedTime;
				}
				if((timer - ru.erked.spaceflight.random.INF.elapsedTime + time2)/60 <= 0){
					isGameOver = true;
					isVictory = false;
				}
			}
		...
		if(!ru.erked.spaceflight.random.INF.lngRussian){
			if(ru.erked.spaceflight.random.INF.planetLevel >= 50){
				...
				if(!isGameOver)
					header.draw(batch, "Level " + (ru.erked.spaceflight.random.INF.planetLevel + 1), 0.5F*width - 0.5F*header.getWidth("Level " + (ru.erked.spaceflight.random.INF.planetLevel + 1)), 1.5F*height + 0.5F*header.getHeight("A"));
				else if(!isVictory)
			...
			if(rocketS.getY() <= 1.0F*height && startTimer <= 0.0F){
				if(!isGameOver){
					...
					if((rocketTimer - ru.erked.spaceflight.random.INF.elapsedTime + time1)/60 <= 0){
						text.draw(batch, (timer - ru.erked.spaceflight.random.INF.elapsedTime + time2)/60 + " + ",
						...
						text.draw(batch, "0", 
								clock[clockIter].getX() + 1.1F*clock[clockIter].getWidth() + text.getWidth((timer - ru.erked.spaceflight.random.INF.elapsedTime + time2)/60 + " + "),
								clock[clockIter].getY() + 0.5F*clock[clockIter].getHeight() + 0.5F*text.getHeight("A"));
						text.setColor(Color.WHITE);
						text.draw(batch, " sec", 
								clock[clockIter].getX() + 1.1F*clock[clockIter].getWidth() + text.getWidth((timer - ru.erked.spaceflight.random.INF.elapsedTime + time2)/60 + " + " + "0"),
								clock[clockIter].getY() + 0.5F*clock[clockIter].getHeight() + 0.5F*text.getHeight("A"));
					}else{
						if(ru.erked.spaceflight.random.INF.planetLevel >= 50){
							text.draw(batch, 60 + bonusTime + " + ", 
									...
									clock[clockIter].getY() + 0.5F*clock[clockIter].getHeight() + 0.5F*text.getHeight("A"));
							text.setColor(Color.SKY);
							text.draw(batch, (rocketTimer - ru.erked.spaceflight.random.INF.elapsedTime + time1)/60 + "",
									clock[clockIter].getX() + 1.1F*clock[clockIter].getWidth() + text.getWidth(60 + bonusTime + " + "), 
									clock[clockIter].getY() + 0.5F*clock[clockIter].getHeight() + 0.5F*text.getHeight("A"));
							text.setColor(Color.WHITE);
							text.draw(batch, " sec", 
									clock[clockIter].getX() + 1.1F*clock[clockIter].getWidth() + text.getWidth(60 + bonusTime + " + " + (rocketTimer - ru.erked.spaceflight.random.INF.elapsedTime + time1)/60),
									clock[clockIter].getY() + 0.5F*clock[clockIter].getHeight() + 0.5F*text.getHeight("A"));
						}else{
							text.draw(batch, 60 + (ru.erked.spaceflight.random.INF.planetLevel - ru.erked.spaceflight.tech.CurPR.getCurPlanet().getLevel())*5 + bonusTime + " + ",
									clock[clockIter].getX() + 1.1F*clock[clockIter].getWidth(), 
									clock[clockIter].getY() + 0.5F*clock[clockIter].getHeight() + 0.5F*text.getHeight("A"));
							text.setColor(Color.SKY);
							text.draw(batch, (rocketTimer - ru.erked.spaceflight.random.INF.elapsedTime + time1)/60 + "",
									clock[clockIter].getX() + 1.1F*clock[clockIter].getWidth() + text.getWidth(60 + (ru.erked.spaceflight.random.INF.planetLevel - ru.erked.spaceflight.tech.CurPR.getCurPlanet().getLevel())*5 + bonusTime + " + "),
									clock[clockIter].getY() + 0.5F*clock[clockIter].getHeight() + 0.5F*text.getHeight("A"));
							text.setColor(Color.WHITE);
							text.draw(batch, " sec", 
									clock[clockIter].getX() + 1.1F*clock[clockIter].getWidth() + text.getWidth(60 + (ru.erked.spaceflight.random.INF.planetLevel - ru.erked.spaceflight.tech.CurPR.getCurPlanet().getLevel())*5 + bonusTime + " + " + (rocketTimer - ru.erked.spaceflight.random.INF.elapsedTime + time1)/60),
									clock[clockIter].getY() + 0.5F*clock[clockIter].getHeight() + 0.5F*text.getHeight("A"));
						}
					}
				}
			...
			if(ru.erked.spaceflight.random.INF.planetLevel >= 50){
				...
				if(!isGameOver)
					header.draw(batch, "Уровень " + (ru.erked.spaceflight.random.INF.planetLevel + 1), 0.5F*width - 0.5F*header.getWidth("Уровень " + (ru.erked.spaceflight.random.INF.planetLevel + 1)), 1.5F*height + 0.5F*header.getHeight("A"));
				else if(!isVictory)
		...
		for(int i=0;i<5;i++){
			for(int j=0;j<10;j++){
				...
				case 1:case 11:case 21:case 31:case 41:{
					...
					if(!isVanishing) diamond[i][j].setAlpha(1.0F);
					if(!isSwipe) diamond[i][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height - i*0.083765F*width, 0.07615F*width, 0.07615F*width);
					if(actI != -1 && actJ != -1 && gemMatrix[actI][actJ] != 0){
						if(!isSwipe && (gemMatrix[actI][actJ] == 5 ||
								gemMatrix[actI][actJ] == 15 ||
								gemMatrix[actI][actJ] == 25 ||
								gemMatrix[actI][actJ] == 35 ||
								gemMatrix[actI][actJ] == 45) && i == actI && j == actJ)
							diamond[i][j].setBounds(0.085F*width + j*0.083765F*width - 0.005F*width, 0.625F*height - i*0.083765F*width - 0.005F*width, 0.08615F*width, 0.08615F*width);
					}
					diamond[i][j].draw(batch);
					break;
				}case 6:case 16:case 26:case 36:case 46:{
					if(!isVanishing) amber[i][j].setAlpha(1.0F);
					if(!isSwipe) amber[i][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height - i*0.083765F*width, 0.07615F*width, 0.07615F*width);
					if(actI != -1 && actJ != -1 && gemMatrix[actI][actJ] != 0){
						if(!isSwipe && (gemMatrix[actI][actJ] == 6 ||
								gemMatrix[actI][actJ] == 16 ||
								gemMatrix[actI][actJ] == 26 ||
								gemMatrix[actI][actJ] == 36 ||
								gemMatrix[actI][actJ] == 46) && i == actI && j == actJ)
							amber[i][j].setBounds(0.085F*width + j*0.083765F*width - 0.005F*width, 0.625F*height - i*0.083765F*width - 0.005F*width, 0.08615F*width, 0.08615F*width);
					}
					amber[i][j].draw(batch);
					break;
				}case 7:case 17:case 27:case 37:case 47:{
					if(!isVanishing) obsidian[i][j].setAlpha(1.0F);
					if(!isSwipe) obsidian[i][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height - i*0.083765F*width, 0.07615F*width, 0.07615F*width);
					if(actI != -1 && actJ != -1 && gemMatrix[actI][actJ] != 0){
						if(!isSwipe && (gemMatrix[actI][actJ] == 7 ||
								gemMatrix[actI][actJ] == 17 ||
								gemMatrix[actI][actJ] == 27 ||
								gemMatrix[actI][actJ] == 37 ||
								gemMatrix[actI][actJ] == 47) && i == actI && j == actJ)
							obsidian[i][j].setBounds(0.085F*width + j*0.083765F*width - 0.005F*width, 0.625F*height - i*0.083765F*width - 0.005F*width, 0.08615F*width, 0.08615F*width);
					}
					obsidian[i][j].draw(batch);
					break;
				}case 8:case 18:case 28:case 38:case 48:{
					if(!isVanishing) quartz[i][j].setAlpha(1.0F);
					if(!isSwipe) quartz[i][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height - i*0.083765F*width, 0.07615F*width, 0.07615F*width);
					if(actI != -1 && actJ != -1 && gemMatrix[actI][actJ] != 0){
						if(!isSwipe && (gemMatrix[actI][actJ] == 8 ||
								gemMatrix[actI][actJ] == 18 ||
								gemMatrix[actI][actJ] == 28 ||
								gemMatrix[actI][actJ] == 38 ||
								gemMatrix[actI][actJ] == 48) && i == actI && j == actJ)
							quartz[i][j].setBounds(0.085F*width + j*0.083765F*width - 0.005F*width, 0.625F*height - i*0.083765F*width - 0.005F*width, 0.08615F*width, 0.08615F*width);
					}
					quartz[i][j].draw(batch);
					break;
				}case 9:{
					granite[i][j].draw(batch);
					granite[i][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height - i*0.083765F*width, 0.07615F*width, 0.07615F*width);
					break;
				}default:{
					break;
				}
	...
	private void rocketDraw(float delta){
		if(rocketS.getY() <= 1.0F*height && !isGameOver){
			...
			if(!isGameOver){
				...
				if(ru.erked.spaceflight.random.INF.elapsedTime % 5 == 0){
					if(fireIter == 0){
						fireIter = 1;
					}else{
						fireIter = 0;
	...
	private boolean addition(){
		...
		for(int j=0;j<10;j++){
			if(gemMatrix[0][j] == 0){
				check = true;
				gemMatrix[0][j] = rand.nextInt(8) + 1;
				...
				case 1:{
					topaz[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("topaz");
					topaz[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 2:{
					sapphire[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("sapphire");
					sapphire[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 3:{
					ruby[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("ruby");
					ruby[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 4:{
					emerald[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("emerald");
					emerald[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 5:{
					diamond[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("diamond");
					diamond[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 6:{
					amber[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("amber");
					amber[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 7:{
					obsidian[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("obsidian");
					obsidian[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 8:{
					quartz[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("quartz");
					quartz[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 11:{
					topaz[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("topazB");
					topaz[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 21:{
					topaz[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("topazC");
					topaz[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 31:{
					topaz[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("topazL");
					topaz[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 41:{
					topaz[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("topazT");
					topaz[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 12:{
					sapphire[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("sapphireB");
					sapphire[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 22:{
					sapphire[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("sapphireC");
					sapphire[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 32:{
					sapphire[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("sapphireL");
					sapphire[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 42:{
					sapphire[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("sapphireT");
					sapphire[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 13:{
					ruby[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("rubyB");
					ruby[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 23:{
					ruby[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("rubyC");
					ruby[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 33:{
					ruby[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("rubyL");
					ruby[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 43:{
					ruby[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("rubyT");
					ruby[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 14:{
					emerald[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("emeraldB");
					emerald[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 24:{
					emerald[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("emeraldC");
					emerald[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 34:{
					emerald[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("emeraldL");
					emerald[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 44:{
					emerald[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("emeraldT");
					emerald[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 15:{
					diamond[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("diamondB");
					diamond[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 25:{
					diamond[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("diamondC");
					diamond[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 35:{
					diamond[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("diamondL");
					diamond[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 45:{
					diamond[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("diamondT");
					diamond[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 16:{
					amber[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("amberB");
					amber[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 26:{
					amber[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("amberC");
					amber[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 36:{
					amber[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("amberL");
					amber[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 46:{
					amber[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("amberT");
					amber[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 17:{
					obsidian[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("obsidianB");
					obsidian[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 27:{
					obsidian[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("obsidianC");
					obsidian[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 37:{
					obsidian[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("obsidianL");
					obsidian[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 47:{
					obsidian[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("obsidianT");
					obsidian[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 18:{
					quartz[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("quartzB");
					quartz[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 28:{
					quartz[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("quartzC");
					quartz[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 38:{
					quartz[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("quartzL");
					quartz[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}case 48:{
					quartz[0][j] = ru.erked.spaceflight.random.RES.atlas.createSprite("quartzT");
					quartz[0][j].setBounds(0.085F*width + j*0.083765F*width, 0.625F*height, 0.07615F*width, 0.07615F*width);
					break;
				}default:{
					break;
			...
			for(int j=0;j<10;j++){
				if(route1[i][j] && routeCount1 > 2){
					...
					case 1:case 11:case 21:case 31:case 41:{
						...
						diamond[i][j].setAlpha(gemVan);
						break;
					}case 6:case 16:case 26:case 36:case 46:{
						amber[i][j].setAlpha(gemVan);
						break;
					}case 7:case 17:case 27:case 37:case 47:{
						obsidian[i][j].setAlpha(gemVan);
						break;
					}case 8:case 18:case 28:case 38:case 48:{
						quartz[i][j].setAlpha(gemVan);
						break;
					}default:{
						break;
					}
	...
	private void swap(int i, int j, int a, int b, boolean isGravity){
		//
		if(quartz[i][j] != null && quartz[a][b] == null){
			quartz[a][b] = quartz[i][j];
			quartz[i][j] = null;
		}else if(quartz[a][b] != null && quartz[i][j] == null){
			quartz[i][j] = quartz[a][b];
			quartz[a][b] = null;
		}
		if(obsidian[i][j] != null && obsidian[a][b] == null){
			obsidian[a][b] = obsidian[i][j];
			obsidian[i][j] = null;
		}else if(obsidian[a][b] != null && obsidian[i][j] == null){
			obsidian[i][j] = obsidian[a][b];
			obsidian[a][b] = null;
		}
		if(amber[i][j] != null && amber[a][b] == null){
			amber[a][b] = amber[i][j];
			amber[i][j] = null;
		}else if(amber[a][b] != null && amber[i][j] == null){
			amber[i][j] = amber[a][b];
			amber[a][b] = null;
		}
		if(diamond[i][j] != null && diamond[a][b] == null){
			diamond[a][b] = diamond[i][j];
			diamond[i][j] = null;
		}else if(diamond[a][b] != null && diamond[i][j] == null){
			diamond[i][j] = diamond[a][b];
			diamond[a][b] = null;
		}
	...
	private void swipe(int[] i, int[] j, int[] a, int[] b, int n, float speed){
		...
		for(int f=0;f<n;f++){
			...
			if(quartz[i[f]][j[f]] != null && quartz[a[f]][b[f]] == null){
				if(quartz[i[f]][j[f]].getX() - speed > x2)
					quartz[i[f]][j[f]].setX(quartz[i[f]][j[f]].getX() - speed);
				else if(quartz[i[f]][j[f]].getX() + speed < x2)
					quartz[i[f]][j[f]].setX(quartz[i[f]][j[f]].getX() + speed);
				else if(quartz[i[f]][j[f]].getY() - speed > y2)
					quartz[i[f]][j[f]].setY(quartz[i[f]][j[f]].getY() - speed);
				else if(quartz[i[f]][j[f]].getY() + speed < y2)
					quartz[i[f]][j[f]].setY(quartz[i[f]][j[f]].getY() + speed);
				else{
					quartz[i[f]][j[f]].setX(x2);
					quartz[i[f]][j[f]].setY(y2);
					swipeIter++;
					swap(i[f], j[f], a[f], b[f], isGravity);
				}
			}else if(quartz[a[f]][b[f]] != null && quartz[i[f]][j[f]] == null){
				if(quartz[a[f]][b[f]].getX() - speed > x1)
					quartz[a[f]][b[f]].setX(quartz[a[f]][b[f]].getX() - speed);
				else if(quartz[a[f]][b[f]].getX() + speed < x1)
					quartz[a[f]][b[f]].setX(quartz[a[f]][b[f]].getX() + speed);
				else if(quartz[a[f]][b[f]].getY() - speed > y1)
					quartz[a[f]][b[f]].setY(quartz[a[f]][b[f]].getY() - speed);
				else if(quartz[a[f]][b[f]].getY() + speed < y1)
					quartz[a[f]][b[f]].setY(quartz[a[f]][b[f]].getY() + speed);
				else{
					quartz[a[f]][b[f]].setX(x1);
					quartz[a[f]][b[f]].setY(y1);
					swipeIter++;
					swap(i[f], j[f], a[f], b[f], isGravity);
				}
			}
			if(obsidian[i[f]][j[f]] != null && obsidian[a[f]][b[f]] == null){
				if(obsidian[i[f]][j[f]].getX() - speed > x2)
					obsidian[i[f]][j[f]].setX(obsidian[i[f]][j[f]].getX() - speed);
				else if(obsidian[i[f]][j[f]].getX() + speed < x2)
					obsidian[i[f]][j[f]].setX(obsidian[i[f]][j[f]].getX() + speed);
				else if(obsidian[i[f]][j[f]].getY() - speed > y2)
					obsidian[i[f]][j[f]].setY(obsidian[i[f]][j[f]].getY() - speed);
				else if(obsidian[i[f]][j[f]].getY() + speed < y2)
					obsidian[i[f]][j[f]].setY(obsidian[i[f]][j[f]].getY() + speed);
				else{
					obsidian[i[f]][j[f]].setX(x2);
					obsidian[i[f]][j[f]].setY(y2);
					swipeIter++;
					swap(i[f], j[f], a[f], b[f], isGravity);
				}
			}else if(obsidian[a[f]][b[f]] != null && obsidian[i[f]][j[f]] == null){
				if(obsidian[a[f]][b[f]].getX() - speed > x1)
					obsidian[a[f]][b[f]].setX(obsidian[a[f]][b[f]].getX() - speed);
				else if(obsidian[a[f]][b[f]].getX() + speed < x1)
					obsidian[a[f]][b[f]].setX(obsidian[a[f]][b[f]].getX() + speed);
				else if(obsidian[a[f]][b[f]].getY() - speed > y1)
					obsidian[a[f]][b[f]].setY(obsidian[a[f]][b[f]].getY() - speed);
				else if(obsidian[a[f]][b[f]].getY() + speed < y1)
					obsidian[a[f]][b[f]].setY(obsidian[a[f]][b[f]].getY() + speed);
				else{
					obsidian[a[f]][b[f]].setX(x1);
					obsidian[a[f]][b[f]].setY(y1);
					swipeIter++;
					swap(i[f], j[f], a[f], b[f], isGravity);
				}
			}
			if(amber[i[f]][j[f]] != null && amber[a[f]][b[f]] == null){
				if(amber[i[f]][j[f]].getX() - speed > x2)
					amber[i[f]][j[f]].setX(amber[i[f]][j[f]].getX() - speed);
				else if(amber[i[f]][j[f]].getX() + speed < x2)
					amber[i[f]][j[f]].setX(amber[i[f]][j[f]].getX() + speed);
				else if(amber[i[f]][j[f]].getY() - speed > y2)
					amber[i[f]][j[f]].setY(amber[i[f]][j[f]].getY() - speed);
				else if(amber[i[f]][j[f]].getY() + speed < y2)
					amber[i[f]][j[f]].setY(amber[i[f]][j[f]].getY() + speed);
				else{
					amber[i[f]][j[f]].setX(x2);
					amber[i[f]][j[f]].setY(y2);
					swipeIter++;
					swap(i[f], j[f], a[f], b[f], isGravity);
				}
			}else if(amber[a[f]][b[f]] != null && amber[i[f]][j[f]] == null){
				if(amber[a[f]][b[f]].getX() - speed > x1)
					amber[a[f]][b[f]].setX(amber[a[f]][b[f]].getX() - speed);
				else if(amber[a[f]][b[f]].getX() + speed < x1)
					amber[a[f]][b[f]].setX(amber[a[f]][b[f]].getX() + speed);
				else if(amber[a[f]][b[f]].getY() - speed > y1)
					amber[a[f]][b[f]].setY(amber[a[f]][b[f]].getY() - speed);
				else if(amber[a[f]][b[f]].getY() + speed < y1)
					amber[a[f]][b[f]].setY(amber[a[f]][b[f]].getY() + speed);
				else{
					amber[a[f]][b[f]].setX(x1);
					amber[a[f]][b[f]].setY(y1);
					swipeIter++;
					swap(i[f], j[f], a[f], b[f], isGravity);
				}
			}
			if(diamond[i[f]][j[f]] != null && diamond[a[f]][b[f]] == null){
				if(diamond[i[f]][j[f]].getX() - speed > x2)
					diamond[i[f]][j[f]].setX(diamond[i[f]][j[f]].getX() - speed);
				else if(diamond[i[f]][j[f]].getX() + speed < x2)
					diamond[i[f]][j[f]].setX(diamond[i[f]][j[f]].getX() + speed);
				else if(diamond[i[f]][j[f]].getY() - speed > y2)
					diamond[i[f]][j[f]].setY(diamond[i[f]][j[f]].getY() - speed);
				else if(diamond[i[f]][j[f]].getY() + speed < y2)
					diamond[i[f]][j[f]].setY(diamond[i[f]][j[f]].getY() + speed);
				else{
					diamond[i[f]][j[f]].setX(x2);
					diamond[i[f]][j[f]].setY(y2);
					swipeIter++;
					swap(i[f], j[f], a[f], b[f], isGravity);
				}
			}else if(diamond[a[f]][b[f]] != null && diamond[i[f]][j[f]] == null){
				if(diamond[a[f]][b[f]].getX() - speed > x1)
					diamond[a[f]][b[f]].setX(diamond[a[f]][b[f]].getX() - speed);
				else if(diamond[a[f]][b[f]].getX() + speed < x1)
					diamond[a[f]][b[f]].setX(diamond[a[f]][b[f]].getX() + speed);
				else if(diamond[a[f]][b[f]].getY() - speed > y1)
					diamond[a[f]][b[f]].setY(diamond[a[f]][b[f]].getY() - speed);
				else if(diamond[a[f]][b[f]].getY() + speed < y1)
					diamond[a[f]][b[f]].setY(diamond[a[f]][b[f]].getY() + speed);
				else{
					diamond[a[f]][b[f]].setX(x1);
					diamond[a[f]][b[f]].setY(y1);
					swipeIter++;
					swap(i[f], j[f], a[f], b[f], isGravity);
				}
			}
