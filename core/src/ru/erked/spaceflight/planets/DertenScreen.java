...
public class DertenScreen implements Screen{
	...
	public DertenScreen(final StartSFlight game){
		this.game = game;
	}
	...
	public void show() {
		...
		backgroundSprite = RES.atlas.createSprite("derten");
		backgroundSprite.setBounds(0.0F, 0.0F, width, 2.0F*height);
		
		if(INF.planetLevel >= 30){
			timer = 60*60;
			...
			rocketTimer = CurPR.getCurRocket().getTimeBonus()*60 + 60;
			gemsToWin = 220 + (INF.planetLevel - CurPR.getCurPlanet().getLevel())*15;
		}
	...
	private void gemInit(){
		...
		for(int i=0;i<5;i++){
			for(int j=0;j<10;j++){
				gemMatrix[i][j] = rand.nextInt(6) + 1;
				...
				if(INF.planetLevel == 24){
					gemMatrix[4][0] = 9;
					gemMatrix[4][1] = 9;
					gemMatrix[4][8] = 9;
					gemMatrix[4][9] = 9;
				}else if(INF.planetLevel == 29){
					gemMatrix[3][0] = 9;
					gemMatrix[4][0] = 9;
					gemMatrix[4][1] = 9;
					gemMatrix[3][9] = 9;
	...
	private boolean addition(){
		...
		for(int j=0;j<10;j++){
			if(gemMatrix[0][j] == 0){
				check = true;
				gemMatrix[0][j] = rand.nextInt(6) + 1;
